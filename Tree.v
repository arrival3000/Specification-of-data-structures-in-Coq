...
Proof.
  move => T tree.
  elim: tree => [| Ltree iHl value Rtree iHr HNill] // /=.
Qed.
...
Eval compute in (count (Nill nat)).
Eval compute in (count (Node (Nill nat) (Nill nat) 5 (Nill nat))).
...
Proof.
  move => T tree.
  elim: tree => [| _ _ Ltree iHl value Rtree iHr] // /=.
...
Lemma Lst_elem : forall (tp tl tr : BinaryTree nat) v, Lst (Node tp tl v tr) = tl.
Proof.
  done.
Qed.

Lemma Rst_elem : forall (tp tl tr : BinaryTree nat) v, Rst (Node tp tl v tr) = tr.
Proof.
  done.
Qed.
...
Fixpoint BST' (tree : BinaryTree nat) : bool := 
  match tree with
  | Nill => true
...
Lemma BST'_Nill : BST' null.
Proof.
  done.
Qed.

Lemma BST'_Combine : forall tp tl v tr,
  Left_cond (Node tp tl v tr) -> Right_cond (Node tp tl v tr) -> 
  BST' tl -> BST' tr -> BST' (Node tp tl v tr).
Proof.
  move => /= tp tl value tr Lc Rc lH rH.
...
Lemma BST'_One : forall y, BST' (null -| y |- null).
Proof.
  done.
Qed.

Lemma BST'_Example_1 : BST' ((@ 3) -| 5 |- (@ 7)).
Proof.
  done.
Qed.

Lemma BST'_Two_l : forall a b, b < a -> 
  BST' ((@ b) -| a |- null).
Proof.
  move => /= a b cond.
  solve.
...
Lemma BST'_Two_r : forall a b, a < b -> 
  BST' (null -| a |- (@ b)).
Proof.
  move => /= a b cond; solve.
Qed.

Lemma BST'_Tree : forall a b c, b < a < c -> 
  BST' ((@ b) -| a |- (@ c)).
Proof.
...
Lemma BST'_Example_2 : BST' ((null -| 3 |- (@ 4)) -| 5 |- (@ 7)).
Proof.
  done.
Qed.

Theorem Safety_BST'_insert : forall tree y, BST' tree -> BST' (y >> tree).
Proof.
...
Lemma bstP (tree : BinaryTree nat) : reflect (BST tree) (BST' tree).
Proof.
...
Definition BSTwP' (tree : BinaryTree nat) : bool :=
  BST' tree && Parent_cond tree.

Lemma bstwpP (tree : BinaryTree nat) : reflect (BSTwP tree) (BSTwP' tree).
Proof.
  unfold BSTwP'.
...
Lemma bst_insertP (tree : BinaryTree nat) (y : nat) : 
  reflect (BST' (y >> tree)) (BST' (y >>wP tree)).
Proof.
